...
public:
   ...
   */
  Buffer(unsigned int max_count, size_t size_, class_id_t default_class_id = 0)
      : max_count_(max_count), size_(size_),
        default_class_id_(default_class_id) {
    buffer_ = vector<unsigned int>(size_);
  }
  void add(class_id_t class_id);
  class_id_t get();
  [[nodiscard]] size_t size() const { return buffer_.size(); }
  unsigned int operator[](class_id_t class_id) const {
    return buffer_[class_id];
  }
};
...
class Controller {
  ...
  constexpr static milliseconds WAIT_DEVICE_STAT_ = milliseconds(100);
  ...
  std::unique_ptr<GenericDevice> device_;
  bool dry_run_;
  ...
  /**
   * continuously query device's battery status and save value in `battery_stat`
   * attribute.
   */
  void update_device_stat_();
...
public:
   ...
   */
  Controller(std::unique_ptr<GenericDevice> device, bool dry_run,
             const string &face_detector_path,
             const string &gesture_detector_path,
             const string &name = "CONTROLLER")
      : device_(std::move(device)), dry_run_(dry_run),
        face_detector_(face_detector_path),
        gesture_detector_(gesture_detector_path), buffer_(buffer_len_, 19),
        name_(name) {
   ...
   * @param frame_refresh_rate indicates how often to refresh application window
   * (in ms).
   */
